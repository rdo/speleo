...
public class PlatformerLevel extends World {
	...
	public PlatformerLevel(float x1, float y1, float x2, float y2, float g,
			float iterations) {
		...
		for (int i = 0; i < map.getObjectGroupCount(); i++) {
			for (int j = 0; j < map.getObjectCount(i); j++) {
				...
				if (name.equals("player")) {
					player = new Player(this, x, y, stats);
				} else if (name.equals("enemy")) {
					String strategyName = map.getObjectProperty(i, j,
							"strategy", "TurretStrategy");
					NPC bot = new NPC(this, x, y,
							AbstractNpcStrategy.createStrategy(strategyName));
					npcs.add(bot);
				} else {
					try {
						String packageName = AbstractObject.class.getPackage()
								.getName();
						String className = packageName + "." + name;
						Class clazz = Class.forName(className);
						Object o = clazz.getConstructors()[0].newInstance(this,
								x, y);
	...
	public void addFloor(int x, int y, int width, int height) {
		...
		Body body = new Body(new Rectangle(width, height), xCenter, yCenter,
				true);
		add(body);
