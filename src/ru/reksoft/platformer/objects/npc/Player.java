...
public class Player extends Person implements LightSource, Controllable {
	...
	public Player(PlatformerLevel world, int x, int y, PersonStats stats) {
		...
		for (int i = 0; i < 14; i++) {
			moveLeft.addFrame(
					playerTileset.getSubImage(0, 112 + 56 * i, 52, 56), 80);
		}

		moveRight = new Animation(true);
		for (int i = 0; i < 14; i++) {
			moveRight.addFrame(
					playerTileset.getSubImage(52, 112 + 56 * i, 52, 56), 80);
		}
	...
	public void draw(Graphics g, int x, int y) {
		if (onJoint) {
			g.drawLine(x, y, hook.getX() - world.xOffset, hook.getY()
					+ world.yOffset);
		}
	...
	public void createJoint(float x, float y) {
		...
		j.initialize(body.getJBoxBody(), hook.getJBoxBody(), body.getJBoxBody()
				.getWorldCenter(), hook.getJBoxBody().getWorldCenter());
		j.length = 50;
